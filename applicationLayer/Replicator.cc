...
void Replicator::inner_body(void)
{
    while(1)
    {
        ...
        {
			//~ cout<< name() << " - Replicator passivates" <<endl; fflush(stdout);
            passivate();
        }

        //~ cout<< name() << " - Replicator goes to hold" <<endl; fflush(stdout);
        ...
        hold(DELTA_T);
        //~ cout<< name() << " - Replicator wakes up" <<endl; fflush(stdout);
		if(ends || (*(*tlc)->observer)->get_end())
        {
            passivate();
        }
        //~ cout << "Termina hold replicator " << name()  << endl;
